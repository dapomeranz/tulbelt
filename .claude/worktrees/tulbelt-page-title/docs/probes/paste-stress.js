...
(() => {
  ...
  function readWhen() {
    for (const select of document.querySelectorAll("select")) {
      ...
      if (!text) continue;
      if (
        /is (started|completed|cancelled|opened|closed|pressed|exited|selected)|timer|device|machine|event occurs|input changes/i.test(
          text,
        )
      ) {
  ...
  window.__stress = {
    ...
    async run({ sourceFilter, destinationFilter, settleMs = 1800 } = {}) {
      ...
      const destinations = destinationsHere().filter(
        (d) =>
          !destinationFilter || d.section.toLowerCase().includes(destinationFilter.toLowerCase()),
      );
