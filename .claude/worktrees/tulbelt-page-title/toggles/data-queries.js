...
(() => {
  ...
  async function run({ reloadSchema = false } = {}) {
    ...
    const { search, error } = model.queryToParams({
      ...state.query,
    ...
    try {
      ...
      state.result = {
        ...
        truncated: rows.length >= (state.query.limit ?? 50),
      };
    } catch (err) {
  ...
  function renderResult(result) {
    ...
    if (result.truncated)
      summary.appendChild(el("span", "dq-note", "limit reached — raise it to see more"));
    wrap.appendChild(summary);

    if (count === 0) {
      wrap.appendChild(
        el("p", "dq-hint", "Nothing matched. Loosen a filter, or switch Match all to Match any."),
      );
      return wrap;
    }
