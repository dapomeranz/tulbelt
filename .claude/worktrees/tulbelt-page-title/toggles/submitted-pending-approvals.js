...
(() => {
  ...
  function scrapePerformanceEntries() {
    const d = document.documentElement.dataset;
    const APPS_PENDING_RE =
      /\/api\/apps\/v1\/w\/(\d+)\/apps-pending-approval[/?].*[?&]userId=([^&\s]+)/;
  ...
  function requireSession() {
    ...
    if (!s.auth || !s.userId || !s.wsId) {
      const missing = [!s.auth && "auth token", !s.userId && "user ID", !s.wsId && "workspace ID"]
        ...
        .join(", ");
      throw new Error(
        `Session not ready (missing: ${missing}) — hard-reload the page and try again.`,
      );
    }
    return s;
  }
  ...
  function isPendingApprovalPage() {
    return (
      PENDING_APPROVAL_URL_RE.test(location.search) || PENDING_APPROVAL_URL_RE.test(location.href)
    );
  }
    ...
    async function switchTo(view) {
      ...
      if (!panel) {
        ...
        if (wrapper) {
          ...
          const anchor = heading.closest("section, main, [role='main']") ?? heading.parentElement;
  ...
  function isContextValid() {
    try {
      return Boolean(globalThis.chrome?.runtime?.id ?? globalThis.browser?.runtime?.id);
    } catch (_) {
      return false;
    }
  }
