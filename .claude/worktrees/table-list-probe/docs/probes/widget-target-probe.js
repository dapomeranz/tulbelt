...
(() => {
  ...
  const INTERESTING =
    /^(_id|id|type|widgetType|customWidgetId|eventName|events|event|widget|name)$/;
  const MAX_DEPTH = 24;
  ...
  function summarize(props) {
    ...
    for (const key of Object.keys(props)) {
      ...
      if (typeof value === "string") {
        ...
        if (named)
          out[key] = "[" + value.length + "] " + JSON.stringify(value.slice(0, 3)).slice(0, 400);
  ...
  window.__widgetProbe = () => {
    ...
    if (!selected) {
      ...
      report.found.anyCanvasWidgets = [
        ...document.querySelectorAll('#cssCanvas [data-testid="widget"]'),
      ]
        .slice(0, 4)
        .map(describe);
    } else {
